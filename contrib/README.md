...
A Linux bash script that will set up traffic control (tc) to limit the outgoing bandwidth for connections to the RAIA Network. This means one can have an always-on raianetworkd instance running, and another local raianetworkd/raianetwork-qt instance which connects to this node and receives blocks from it.
