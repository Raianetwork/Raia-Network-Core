...
#include "raianetworkgui.h"

#include "raianetworkunits.h"
...
const std::string RaianetworkGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString RaianetworkGUI::DEFAULT_WALLET = "~Default";

RaianetworkGUI::RaianetworkGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
    clientModel(0),
    walletFrame(0),
...
void RaianetworkGUI::createActions()
{
    QFont font = QFont();
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a raianetwork address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and raianetwork: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to RAIA addresses"));
...
//    restrictedAssetAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    lockWalletAction->setStatusTip(tr("Lock the encrypted wallet to prevent spending and asset issue"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your raianetwork addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified raianetwork addresses"));
    ...
    openAction->setStatusTip(tr("Open a Raianetwork: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible raianetwork command-line options").arg(tr(PACKAGE_NAME)));
...
void RaianetworkGUI::createMenuBar()
{
#ifdef Q_OS_MAC
...
void RaianetworkGUI::createToolBars()
{
    if(walletFrame)
    {
        /** RAIANETWORK START */
        ...
        label->setPixmap(QPixmap::fromImage(QImage(":/icons/raianetworkcointext")));
...
//        toolbar->addAction(votingAction);
        ...
        QVBoxLayout* raianetworkLabelLayout = new QVBoxLayout(toolbarWidget);
        raianetworkLabelLayout->addWidget(label);
        raianetworkLabelLayout->addWidget(toolbar);
        raianetworkLabelLayout->setDirection(QBoxLayout::TopToBottom);
        raianetworkLabelLayout->addStretch(1);
        ...
        labelCurrentMarket->setText(tr("Raianetwork Market Price"));
        ...
        QLabel* labelBtcRaia = new QLabel();
        labelBtcRaia->setText("BTC / RAIA");
        labelBtcRaia->setContentsMargins(15,0,0,0);
        labelBtcRaia->setFixedHeight(75);
        labelBtcRaia->setAlignment(Qt::AlignVCenter);
        labelBtcRaia->setStyleSheet(STRING_LABEL_COLOR);
        labelBtcRaia->setFont(currentMarketFont);
        ...
        priceLayout->addWidget(labelBtcRaia, 0 , Qt::AlignVCenter | Qt::AlignLeft);
        ...
        // Get the latest Raianetwork release and let the user know if they are using the latest version
        // Network request code for the header widget
...
void RaianetworkGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool RaianetworkGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool RaianetworkGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void RaianetworkGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void RaianetworkGUI::setWalletActionsEnabled(bool enabled)
{
...
void RaianetworkGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void RaianetworkGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void RaianetworkGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void RaianetworkGUI::optionsClicked()
{
...
void RaianetworkGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void RaianetworkGUI::showDebugWindow()
{
...
void RaianetworkGUI::showDebugWindowActivateInformation()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_INFO);
    showDebugWindow();
}

void RaianetworkGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void RaianetworkGUI::showDebugWindowPeerList()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_PEERS);
    showDebugWindow();
}

void RaianetworkGUI::showDebugWindowNetworkTraffic()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_GRAPH);
    showDebugWindow();
}

void RaianetworkGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void RaianetworkGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void RaianetworkGUI::openClicked()
{
...
void RaianetworkGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void RaianetworkGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void RaianetworkGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void RaianetworkGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void RaianetworkGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void RaianetworkGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** RAIANETWORK START */
void RaianetworkGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void RaianetworkGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void RaianetworkGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void RaianetworkGUI::gotoRestrictedAssetsPage()
{
...
void RaianetworkGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to raianetwork network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void RaianetworkGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void RaianetworkGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void RaianetworkGUI::updateHeadersSyncProgressLabel()
{
...
void RaianetworkGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void RaianetworkGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("Raianetwork"); // default title
...
void RaianetworkGUI::changeEvent(QEvent *e)
{
...
void RaianetworkGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void RaianetworkGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void RaianetworkGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "RAIA")
        msg += tr("Amount: %1\n").arg(RaianetworkUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(RaianetworkUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void RaianetworkGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void RaianetworkGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool RaianetworkGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool RaianetworkGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void RaianetworkGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void RaianetworkGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void RaianetworkGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void RaianetworkGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void RaianetworkGUI::detectShutdown()
{
...
void RaianetworkGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void RaianetworkGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void RaianetworkGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(RaianetworkGUI *gui, const std::string& message, const std::string& caption, unsigned int style)
{
...
static bool ThreadSafeMnemonic(RaianetworkGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void RaianetworkGUI::subscribeToCoreSignals()
{
...
void RaianetworkGUI::unsubscribeFromCoreSignals()
{
...
void RaianetworkGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void RaianetworkGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<RaianetworkUnits::Unit> units = RaianetworkUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const RaianetworkUnits::Unit unit : units)
    {
    	#ifndef QTversionPreFiveEleven
        	max_width = qMax(max_width, fm.horizontalAdvance(RaianetworkUnits::name(unit)));
        #else
        	max_width = qMax(max_width, fm.width(RaianetworkUnits::name(unit)));
        #endif
    }
...
{
    menu = new QMenu(this);
    for (RaianetworkUnits::Unit u : RaianetworkUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(RaianetworkUnits::name(u)), this);
...
{
    setText(RaianetworkUnits::name(newUnits));
}
...
void RaianetworkGUI::getPriceInfo()
{
    request->setUrl(QUrl("https://api.binance.com/api/v1/ticker/price?symbol=RAIABTC"));
    networkManager->get(*request);
}

#ifdef ENABLE_WALLET
void RaianetworkGUI::mnemonic()
{
...
void RaianetworkGUI::getLatestVersion()
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/Raianetwork/Raia-Network-core/releases"));
