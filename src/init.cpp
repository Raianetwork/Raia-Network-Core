 ...
 * between raianetworkd, and raianetwork-qt and non-server tools.
*/
...
void PrepareShutdown()
{
    ...
    RenameThread("raianetwork-shutoff");
    ...
    FlushWallets();
#endif
    GenerateRaianetworks(false, 0, GetParams());
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), RAIANETWORK_CONF_FILENAME));
    if (mode == HMM_RAIANETWORKD)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), RAIANETWORK_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/Raianetwork/Raia-Network-core>";
    const std::string URL_WEBSITE = "<https://raianetwork.xyz>";
...
void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("raianetwork-loadblk");
...
/** Sanity checks
 *  Ensure that Raianetwork is running in a usable environment with all
...
void InitLogging()
{
    ...
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Raianetwork version %s\n", FormatFullVersion());
}
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Raianetwork process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", RAIANETWORK_CONF_FILENAME)).string());
    ...
    GenerateRaianetworks(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
