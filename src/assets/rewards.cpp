...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for RAIA
    ...
    if (p_rewardSnapshot.strDistributionAsset != "RAIA") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is RAIA with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using RAIA differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "RAIA") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse Raianetwork address (already validated during ownership snapshot creation)
