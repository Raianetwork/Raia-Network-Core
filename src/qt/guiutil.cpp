...
#include "raianetworkaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a raianetwork address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new RaianetworkAddressEntryValidator(parent));
    widget->setCheckValidator(new RaianetworkAddressCheckValidator(parent));
}
...
bool parseRaianetworkURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no raianetwork: URI
    if(!uri.isValid() || uri.scheme() != QString("raianetwork"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!RaianetworkUnits::parse(RaianetworkUnits::RAIA, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseRaianetworkURI(QString uri, SendCoinsRecipient *out)
{
    // Convert raianetwork:// to raianetwork:
    //
    //    Cannot handle this later, because raianetwork:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("raianetwork://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "raianetwork:");
    }
    QUrl uriInstance(uri);
    return parseRaianetworkURI(uriInstance, out);
}

QString formatRaianetworkURI(const SendCoinsRecipient &info)
{
    QString ret = QString("raianetwork:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(RaianetworkUnits::format(RaianetworkUnits::RAIA, info.amount, false, RaianetworkUnits::separatorNever));
        paramCount++;
    }
...
bool openRaianetworkConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", RAIANETWORK_CONF_FILENAME));

    /* Open raianetwork.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Raianetwork.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Raianetwork (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Raianetwork (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Raianetwork*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "raianetwork.desktop";
    return GetAutostartDir() / strprintf("raianetwork-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a raianetwork.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Raianetwork\n";
        else
            optionFile << strprintf("Name=Raianetwork (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the raianetwork app
...
bool GetStartOnSystemStartup()
{
    CFURLRef raianetworkAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (raianetworkAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, raianetworkAppUrl);

    CFRelease(raianetworkAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add raianetwork app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, raianetworkAppUrl, nullptr, nullptr);
    }
