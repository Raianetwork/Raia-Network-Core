...
The RPM spec file provided here is for Raianetwork-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://raianetwork.xyz/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Raianetwork git repository](https://github.com/Raianetwork/Raia-Network-core) but are not part of
...
Currently only `Source10` is used, for the example `raianetwork.conf` file.
...
in the raianetwork git repository. At present only an SVG version of the Raianetwork
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the raianetwork
...
needed when 0.12.1 is released, a proper fix is already in the Raianetwork git
master branch.
...
idea to build RAIA Network against a static version of that library compiled
...
The `raianetwork.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `raianetwork-server` package. At this point, most RPM based
...
The `raianetwork-server` package should have SELinux support. How to properly do
...
Raianetwork-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/raianetwork/). He has been
packaging Raianetwork for Fedora at least since 2012.
...
4. I renamed the `raianetwork` package that contains the Qt GUI to `raianetwork-core` as
that appears to be how the general population refers to it, in contrast to
`raianetwork-xt` or `raianetwork-classic`. I wanted to make sure the general population
