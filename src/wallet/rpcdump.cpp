...
UniValue importaddress(const JSONRPCRequest& request)
{
    ...
    if (IsValidDestination(dest)) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Raianetwork address or script");
    }
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The raianetwork address for the private key\n"
            "\nResult:\n"
    ...
    return CRaianetworkSecret(vchSecret).ToString();
}
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"filename\"    (string, required) The filename with path (either absolute or relative to raianetworkd)\n"
            "\nResult:\n"
            "{                           (json object)\n"
     ...
     * https://github.com/Raianetwork/Raia-Network-core/issues/9934
    ...
    // produce output
    file << strprintf("# Wallet dump created by Raianetwork %s\n", CLIENT_BUILD);
    ...
    if (!seed_id.IsNull())
    {
        ...
        if (!pwallet->GetHDChain().IsBip44()) {
            ...
            if (pwallet->GetKey(seed_id, seed)) {
                ...
                CRaianetworkExtKey b58extkey;
                b58extkey.SetKey(masterKey);

                CExtPubKey pubkey;
                pubkey = masterKey.Neuter();

                CRaianetworkExtPubKey b58extpubkey;
    ...
    for (std::vector<std::pair<int64_t, CKeyID> >::const_iterator it = vKeyBirth.begin(); it != vKeyBirth.end(); it++) {
        ...
        if (pwallet->GetKey(keyid, key)) {
            file << strprintf("%s %s ", CRaianetworkSecret(key).ToString(), strTime);
    ...
    if (!seed_id.IsNull())
    {
        ...
        if (!pwallet->GetHDChain().IsBip44()) {
            ...
            if (pwallet->GetKey(seed_id, seed)) {
                ...
                // Get the Raianetwork Ext Key from the master key
                CRaianetworkExtKey b58extkey;
                b58extkey.SetKey(masterKey);
                ...
                // Get the Raianetwork Ext Key from the public key
                CRaianetworkExtPubKey b58extpubkey;
                b58extpubkey.SetKey(pubkey);
                ...
                // Create the Raianetwork Account Ext Private Key
                CRaianetworkExtKey b58accountextprivatekey;
                b58accountextprivatekey.SetKey(accountKey);

                // Create the Raianetwork Account Ext Public Key
                CRaianetworkExtPubKey b58actextpubkey;
                    ...
                    result.pushKV(
                        ...
                        JSONRPCError(
                            ...
                            strprintf("Rescan failed for key with creation timestamp %d. There was an error reading a "
                                      ...
                                      "caused by pruning or data corruption (see raianetworkd log for details) and could "
